...
internal class BaseHardwareRegistryTest {
    ...
    fun `fail to unregister registered device`() {
        ...
        val device = registry.registerDevice(deviceId) {
            mock<Device> {
                ...
                on { disconnect() } doReturn IO.raiseError(UnsupportedOperationException(""))
            }
        }.getOrFail()
    ...
    @Test
    fun `register resource with pin group containing pins used in pins by resources in another group`() {
        val device = registry.makeDeviceOrFail()

        val group = registry.registerDeviceResourceGroup(
            device,
            immutableListOf(
                ResourceId(DefaultResourceTypes.Servo, DefaultAttachmentPoints.Pin(1)),
                ResourceId(DefaultResourceTypes.Servo, DefaultAttachmentPoints.Pin(2)),
                ResourceId(DefaultResourceTypes.Servo, DefaultAttachmentPoints.Pin(3))
            ),
            MockUnprovisionedDeviceResourceGroup.create
        )

        val resourceThatFails = registry.registerDeviceResource(
            device,
            ResourceId(
                DefaultResourceTypes.DigitalOut, DefaultAttachmentPoints.PinGroup(byteArrayOf(2, 3))
            ),
            MockUnprovisionedDeviceResource.create
        )

        assertAll(
            { assertTrue(group.isRight(), "group.isRight()") },
            { assertTrue(resourceThatFails.isLeft(), "resourceThatFails.isLeft()") }
        )
    }

    @Test
    fun `register resource with pin group equal to another resource's pin group`() {
        val device = registry.makeDeviceOrFail()
        ...
        val group = registry.registerDeviceResourceGroup(
            ...
            immutableListOf(
                ...
                ResourceId(
                    DefaultResourceTypes.Servo,
                    DefaultAttachmentPoints.PinGroup(byteArrayOf(2, 3))
                ),
                ResourceId(DefaultResourceTypes.Servo, DefaultAttachmentPoints.Pin(4))
            ),
            MockUnprovisionedDeviceResourceGroup.create
        )
    ...
    @Test
    fun `register resource with pin group containing pins used by resources in a group`() {
        val device = registry.makeDeviceOrFail()
        ...
        val resourceThatFails = registry.registerDeviceResource(
            device,
            ResourceId(
                DefaultResourceTypes.DigitalOut, DefaultAttachmentPoints.PinGroup(byteArrayOf(2, 4))
            ),
            MockUnprovisionedDeviceResource.create
        )
    ...
    @Test
    fun `register resource group with pin group containing pins used in pins by resources in another group`() {
        val device = registry.makeDeviceOrFail()
        ...
        val resourceThatFails = registry.registerDeviceResourceGroup(
            device,
            immutableListOf(
                ResourceId(
                    DefaultResourceTypes.DigitalOut,
    ...
    @Test
    fun `register resource group with pin group containing pins used by resources in the same group`() {
        val device = registry.makeDeviceOrFail()
        ...
        val resourceThatFails = registry.registerDeviceResourceGroup(
            ...
            immutableListOf(
                ...
                ResourceId(
                    DefaultResourceTypes.DigitalOut,
                    DefaultAttachmentPoints.PinGroup(byteArrayOf(3, 4))
                )
            ),
            MockUnprovisionedDeviceResourceGroup.create
        )
