import Bowler_cad_gradle.Strings.spotlessLicenseHeaderDelimiter
import Bowler_cad_gradle.Versions.bowlerCadVersion
import Bowler_cad_gradle.Versions.ktlintVersion
...
object Versions {
    ...
    const val bowlerCadVersion = "0.0.4-beta"
}

allprojects {
    version = bowlerCadVersion
...
configure(javaProjects) {
    ...
    spotless {
        java {
            ...
            licenseHeaderFile(
                "${rootProject.rootDir}/config/spotless/bowler-cad.license",
...
configure(publishedProjects) {
    ...
    task<Jar>("sourcesJar") {
        classifier = "sources"
        baseName = "bowler-cad-${this@configure.name.toLowerCase()}"
    ...
    val publicationName = "publication-bowler-cad-${name.toLowerCase()}"

    publishing {
        publications {
            create<MavenPublication>(publicationName) {
                artifactId = "bowler-cad-${this@configure.name.toLowerCase()}"
    ...
    bintray {
        ...
        with(pkg) {
            ...
            vcsUrl = "https://github.com/CommonWealthRobotics/bowler-cad.git"
            githubRepo = "https://github.com/CommonWealthRobotics/bowler-cad"
            with(version) {
                name = bowlerCadVersion
                desc = "CAD for the Bowler stack."
            }
        }
    }
}
