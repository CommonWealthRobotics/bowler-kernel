...
object Main {
    ...
    internal fun dispatch(line: String, commands: List<Command>): String {
        val substrings = line.split(Regex("\\s")).map { it.trim() }.filter { it.isNotEmpty() }
        ...
        if (primaryCommandString.startsWith('?')) {
            ...
            return if (primaryCommandStringWithoutPrefix.isEmpty()) {
                ""
            } else {
                commands.firstOrNull { it.name == primaryCommandStringWithoutPrefix }?.helpMessage()
                    ?: getCommandNotFound(primaryCommandStringWithoutPrefix, commands)
            }
        }

        val args = substrings.subList(1, substrings.size)
    ...
    fun main(args: Array<String>) {
        ...
        val commands = listOf(
            Command(name = "help", help = "Prints information about Bowler and where to find more resources.") {
                """
                    Welcome to Bowler. The manual and many great tutorials and learning resources are available at:
