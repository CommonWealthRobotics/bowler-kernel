...
class InverseKinematicsEngineThetaTest {
    ...
    fun `test compute cmm theta 23`() {
        ...
        val newNewCenter = cmmParams.subList(0, 4)
            .forwardKinematics(arrayOf(newTheta1, newThetas23[1][0], newThetas23[1][1], 0.0).toDoubleArray())
    ...
    fun `test compute cmm elbowUp and elbowDown`() {
        ...
        val wristCenter = cmmParams.subList(0, 4)
            .forwardKinematics(arrayOf(0.0, -PI / 4, -PI / 4 - 0.52933 + 1.38545, 0.0).toDoubleArray())
            .cols(3, 4).rows(0, 3)
        val theta1 = cmmParams.computeTheta1(wristCenter)[0]
        val thetas23 = cmmParams.computeTheta23(wristCenter, theta1)
    ...
    fun `test compute hephaestus theta 23`() {
        val wristCenter =
            hephaestusParams.forwardKinematics(arrayOf(0.0, -PI / 4, PI / 4 + 0.81979, 0.0).toDoubleArray())
                .cols(3, 4).rows(0, 3)
        ...
        val newCenterElbowUp =
            hephaestusParams.forwardKinematics(arrayOf(theta1, thetas23[0][0], thetas23[0][1], 0.0).toDoubleArray())
        val newCenterElbowDown =
            hephaestusParams.forwardKinematics(arrayOf(theta1, thetas23[1][0], thetas23[1][1], 0.0).toDoubleArray())
    ...
    fun `test compute puma theta 23`() {
        ...
        val newNewCenter = pumaParams.subList(0, 4)
            .forwardKinematics(arrayOf(newTheta1, newThetas23[0][0], newThetas23[0][1], 0.0).toDoubleArray())
